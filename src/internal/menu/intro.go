...
import (
	...
	"runa/src/internal/cine"
	"runa/src/internal/spell"
...
func ShowStory(in io.Reader, out io.Writer, path string) (*character.Character, error) {
	...
	// Fond video partage : une frame par dessin, en boucle.
	frame := 0
	...
	if tui.DialogueKeys(c, out, keys, "STORY", behind, storyTexts...) {
	...
	// Quitter ici ne fait qu'avancer (comme avant) : on garde le perso.
	destinyTexts := []string{
	...
	tui.DialogueKeys(c, out, keys, "DESTINY", behind, destinyTexts...)
	return ch, nil
}
